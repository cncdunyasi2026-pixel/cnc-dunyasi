...
import { loadSparePartBrowseProfiles } from "@/services/marketplaceBrowseService";
...
export default function SparePartPage() {
  ...
  const [adminBrands, setAdminBrands] = useState<string[]>([]);

  useEffect(() => {
    void loadSparePartBrowseProfiles()
      .then(setAllItems)
      .finally(() => setLoadingData(false));
    void sparePartCategoryService.getAll().then((list) => setAdminCategories(list.map((i) => i.name)));
    void sparePartBrandService.getAll().then((list) => setAdminBrands(list.map((i) => i.name)));
  }, []);

  const filterGroups = useMemo<FilterGroup[]>(() => {
    const count = (field: (keyof MarketplaceProfile), val: string) =>
      allItems.filter((i) => i[field] === val).length;

    const partCats = adminCategories.length > 0
      ? adminCategories
      : ([...new Set(allItems.map((i) => i.partCategory).filter(Boolean))] as string[]);

    const brands = adminBrands.length > 0
      ? adminBrands
      : ([...new Set(allItems.map((i) => i.brandCompat).filter(Boolean))] as string[]);

    // Uzmanlık alanı: marka listesiyle aynı admin kaynağı
    const expertiseOpts = adminBrands.length > 0
      ? adminBrands
      : ([...new Set(allItems.map((i) => i.expertise).filter(Boolean))] as string[]);

    const stockOptions = ["Stokta Var", "Sipariş Üzerine"];

    const groups: FilterGroup[] = [];
    if (partCats.length > 0) groups.push({
      id: "kategori", label: "Parça Kategorisi",
      options: partCats.map((c) => ({ label: c, count: count("partCategory", c) || undefined })),
    });
    if (brands.length > 0) groups.push({
      id: "marka", label: "Marka Uyumu",
      options: brands.map((b) => ({ label: b, count: count("brandCompat", b) || undefined })),
    });
    if (expertiseOpts.length > 0) groups.push({
      id: "uzmanlik", label: "Uzmanlık Alanı",
      defaultCollapsed: true,
      options: expertiseOpts.map((e) => ({
        label: e,
        count: allItems.filter((i) => i.expertise === e).length || undefined,
      })),
    });
    groups.push({
      id: "stok", label: "Stok Durumu", defaultCollapsed: true,
      options: stockOptions.map((s) => ({ label: s, count: count("stockStatus", s) || undefined })),
    });
    ...
    return groups;
  }, [allItems, adminCategories, adminBrands]);
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 py-6">
      ...
      <div className="full-bleed relative mb-6">
        <img src="/yedek_parca.png" alt="Yedek parca banner" className="h-[230px] w-full object-cover sm:h-[280px]" />
        <div className="absolute inset-0 bg-[#0F2A4A]/65" />
        <div className="absolute inset-0 flex items-center justify-center px-4 sm:px-8">
          <div className="max-w-3xl text-center text-white">
            <p className="text-[10px] font-semibold tracking-[0.18em] text-[#7A8CA5] sm:text-xs">CNC DUNYASI TEDARİK AĞI</p>
            <h1 className="mt-2 text-2xl font-extrabold leading-tight sm:text-4xl">Doğru Yedek Parçaya Tek Noktadan Ulaş</h1>
            <p className="mt-2 text-sm text-white/85 sm:text-base">
              Tedarikçi firmaları karşılaştır, stok ve uzmanlık bilgilerine hızla eriş.
            </p>
            <Link href="/ilan-ver/yedek-parca"
        ...
        <div className="min-w-0">
          {loadingData ? (
            ...
            <MarketplaceBrowsePanel
              ...
              searchPlaceholder="Marka, ürün tipi veya tedarikçi ara..."
      ...
      <div className="mt-8 mb-4 rounded-2xl border border-[#dbe2ea] bg-gradient-to-b from-white to-[#f5f8fc] p-4 shadow-sm sm:p-5">
        <p className="text-xs font-semibold tracking-[0.18em] text-[#7A8CA5]">TEDARİK AĞI</p>
        <h1 className="mt-1 text-2xl font-extrabold text-[#0F2A4A] sm:text-3xl">YEDEK PARÇA FİRMALARI</h1>
        <p className="mt-2 text-sm text-[#5f6f86]">
          Firmalar kendi firma profillerini oluşturur. Kartlardan stok, uzmanlık alanları ve iletişim detaylarını görüntüleyebilirsiniz.
        </p>
      </div>
